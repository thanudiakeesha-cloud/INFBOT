...
- Added a live edited WhatsApp progress message to `.movie` film downloads. The message now shows separate download and upload progress bars plus speed. Large movie sending uses the saved file path instead of reading the whole file into RAM, downloads use a faster keep-alive streaming pipeline, completed file size is verified, concurrent movie downloads are limited by `MAX_MOVIE_DOWNLOADS` (default 1), and files over `MOVIE_UPLOAD_MAX_MB` (default 300MB) are sent as direct links instead of being uploaded through Baileys to prevent Railway out-of-memory crashes and 99% upload freezes.
